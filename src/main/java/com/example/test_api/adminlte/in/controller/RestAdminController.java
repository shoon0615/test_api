package com.example.test_api.adminlte.in.controller;
...
@Tag(name = "RestAdminController", description = "테스트 컨트롤러")
public class RestAdminController {
    ...
    @Operation(summary = "상세정보 조회", description = "상세정보를 조회한다.",
        responses = {
            @ApiResponse(responseCode = "200", description = "성공", content = @Content(schema = @Schema(implementation = AdminResponse.class))),
            @ApiResponse(responseCode = "400", description = "실패", content = @Content(schema = @Schema(implementation = AdminResponse.class))),
            @ApiResponse(responseCode = "404", description = "못찾음", content = @Content(schema = @Schema(implementation = AdminResponse.class)))
    })
    public ResponseEntity getTest(
        ...
        return adminService.findTest(id);
    }

    @GetMapping
    @Operation(summary = "목록 조회", description = "목록을 조회한다.",
        responses = {
            @ApiResponse(responseCode = "200", description = "성공", content = @Content(array = @ArraySchema(schema = @Schema(implementation = AdminResponse.class)))),
            @ApiResponse(responseCode = "400", description = "실패", content = @Content(schema = @Schema(implementation = AdminResponse.class))),
            @ApiResponse(responseCode = "404", description = "못찾음", content = @Content(schema = @Schema(implementation = AdminResponse.class)))
    })
    public ResponseEntity getTestList(@RequestBody final AdminRequest request) {
        return adminService.findAllTest(request);
    }
    ...
    public ResponseEntity saveTest(@RequestBody final AdminRequest request) {
        return adminService.saveTest(request);
    }

    @DeleteMapping("/{id}")
    public ResponseEntity deleteTest(final Long id) {
        return adminService.deleteTest(id);
    }
    ...
    public ResponseEntity getCommonList() {
        return adminService.findAllCommon();
    }
